...
def render_readiness_ring(score: int, label: str) -> None:
    ...
    scanlines = []
    for y in range(center - 72, center + 72 + 1, 4):
        dy = y - center
        half = int((72**2 - dy**2) ** 0.5)
        if half > 0:
            scanlines.append(f'<line x1="{center - half}" y1="{y}" x2="{center + half}" y2="{y}" stroke="rgba(0,0,0,0.15)" stroke-width="1"/>')
    scanlines_str = "".join(scanlines)
